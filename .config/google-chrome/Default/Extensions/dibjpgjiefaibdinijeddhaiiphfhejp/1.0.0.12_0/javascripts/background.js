  ...
  chrome.tabs.query({ active: true, currentWindow: true }, function(tabs) {
    ...
    if (tab && tab.url && tab.url.startsWith('https://mail.google.com/') && tab.status == "complete") {
      chrome.tabs.sendMessage(tab.id, { action: "getUserEmailAddress" }, function(email_or_login) {
        var url_dashboard;
        if (email_or_login) {
          url_dashboard = g_server_url + 'main_gmail_calreserve?email_or_login='+encodeURIComponent(email_or_login)+'&switch_login=1'+'&user_email='+encodeURIComponent(email_or_login)+'&registration_code=via_gmail_calreserve&provider_mode=google_gmail&auto_login=true';
        } else {
          url_dashboard = g_server_url + 'main_gmail_calreserve?registration_code=via_gmail_calreserve&provider_mode=google_gmail&auto_login=true';
        }
        openTab({url: url_dashboard}, url_dashboard);
      });
    } else {
      var url_dashboard = g_server_url + 'main_gmail_calreserve?registration_code=via_gmail_calreserve&provider_mode=google_gmail&auto_login=true';
      openTab({url: url_dashboard}, url_dashboard);
    }
...
chrome.runtime.onConnect.addListener(function(gmail_port) {
  ...
  console.log("gmail_calreserve: gmail_port=",gmail_port);
  ...
  gmail_port.onMessage.addListener(function (msg) {
    ...
    if (msg && msg.action == 'open_gmail_html_popup') {

      var draft_id = msg.draft_id;
      var email_draft_html = msg.email_draft_html;
      var url = msg.url;

      popup_hash_by_port_name[draft_id] = { 'gmail_port': gmail_port, 'email_draft_html': email_draft_html, url: url };
      chrome.windows.create({ url: url, type: "popup", focused: true }, function(win) {
        popup_hash_by_port_name[draft_id]['window_id'] = win.id;
    ...
    } else if (msg && msg.action == 'open_gmail_html_popup_for_my_templates') {

      var item_id = msg.id;
      var email_draft_html = msg.email_draft_html;
      var url = msg.url;

      popup_hash_by_port_name[item_id] = { 'gmail_port': gmail_port, 'email_draft_html': email_draft_html, url: url };
      chrome.windows.create({ url: url, type: "popup", focused: true }, function(win) {
        popup_hash_by_port_name[item_id]['window_id'] = win.id;
        chrome.windows.update(win.id, {
          left: window.left,
          top: window.top,
          width: Math.round(window.width),
    ...
    } else if (msg.type === 'CHQ_REGISTER_USER_ID') {
      ...
      if (popup) {
        ...
        if (popup['popup_port']) {
          ...
          console.log("gmail_calreserve: forward message gmail_port=",gmail_port, " popup_port=", popup_port, " msg=",msg);
  ...
  gmail_port.onDisconnect.addListener(function(gmail_port_disconnected) {
    ...
    if (popup) {
      ...
      // since popup_hash_by_port_name is going to be null we will reply abort on next save causing
      // everything to die
...
function refreshBrowser(target, bring_to_foreground_flag) {
  ...
  chrome.windows.getAll({
    ...
    if (bring_to_foreground_flag && !found_existing_flag) {
      chrome.tabs.create({
        url: 'https://mail.google.com?from_extension=gmail_calreserve#inbox',
        active: true
      });
    }
  });
...
chrome.runtime.setUninstallURL("https://www.cloudhq.net/uninstall_chrome_extension?product_what=gmail_calreserve");
