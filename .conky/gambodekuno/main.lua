...
function conky_main(  )
	...
	draw_image(ir, centerx, centery, face_radius, pathway.."archlinux.png");
	...
	local item_curvey = centery + math.sin(angle) * height/3;
	local item_radius = 20;
	...
	cairo_set_font_size(cr,item_font_size/1.05);
	for i = 1,10 do
		local addison = "                 ";
	...
	for i = 1,no_of_cores do
		local name = "CPU "..i;
		local value = conky_parse("${cpu cpu "..i.."}");
		text = name.."    "..value.."%";
		cairo_move_to(cr, item_endx - extents.width/2, item_endy + item_font_size/1.2 * (i+1) + 5);
		cairo_show_text(cr,text);
	end
	...
	cairo_show_text(cr, text);
		...
		-- Home file system ------------------------------------------------------------ home fs ----------------------------------------------------------------------------

	angle = angle + 40*(math.pi/180);
	item_startx = item_centerx - item_radius - 5;
	item_starty = item_centery;
	item_endx = item_startx + math.cos(angle) * width/8;
	item_endy = item_starty + math.sin(angle) * height/12;
	item_curvex = item_startx + math.cos(angle) * width/12;
	item_curvey = item_starty + math.cos(angle) * height/12;
	local item_centerx = item_endx + math.cos(angle) * (item_radius + 5);
	local item_centery = item_endy + math.sin(angle) * (item_radius + 5);
	...
	cairo_show_text(cr, text);
		...
		-- Var file system ------------------------------------------------------------ var fs ----------------------------------------------------------------------------

	angle = angle + 310*(math.pi/180);
	item_endx = item_startx + math.cos(angle) * width/9;
	item_endy = item_starty + math.sin(angle) * height/10;
	item_curvex = item_startx + math.cos(angle) * width/18;
	item_curvey = item_starty + math.sin(angle) * height/-24;
	local item_centerx = item_endx + math.cos(angle) * (item_radius + 5);
	local item_centery = item_endy + math.sin(angle) * (item_radius + 5);

	-- value of free space and total space
	local free = "Free: "..conky_parse("${fs_free /var}");
	local total = "Total: "..conky_parse("${fs_size /var}");
	...
	local item_endx = centerx + math.cos(angle) * width/6;
	local item_endy = centery + math.sin(angle) * height/6;
	local item_curvex = centerx + math.cos(angle) * width/8;
	local item_curvey = centery + math.sin(angle) * height/24;
	local item_radius = 20;
	local item_centerx = item_endx + math.cos(angle) * (item_radius + 5);
	local item_centery = item_endy + math.sin(angle) * (item_radius + 5);
	local item_font_size = height/50;
	...
	item_endx = item_startx + math.cos(angle) * width/6;
	item_endy = item_starty + math.sin(angle) * height/6;
	item_curvex = item_startx + math.cos(angle) * width/16;
	item_curvey = item_starty + math.sin(angle) * height/10;
	-- print(item_startx.." "..item_starty.." "..item_endx.." "..item_endy.." "..item_curvex.." "..item_curvey)
	...
	-- value text
	text = conky_parse("${execpi 300 checkupdates | wc -l}");
	cairo_text_extents(cr, text, extents)
	cairo_move_to(cr, item_centerx - extents.width/2, item_centery + item_radius + item_font_size + 8);
	cairo_show_text(cr, text);
	...
	-- value text
	text = conky_parse("${battery_short BAT1}");
	if text == "U" then
		text=""
		-- image
		local ir = cairo_create(cs);
	...
	text = conky_parse("${battery_time BAT1}");
	cairo_text_extents(cr, text, extents)
	cairo_move_to(cr, item_centerx - extents.width/2, item_centery + item_radius + item_font_size*2+8);
	cairo_show_text(cr, text);
...
-- network ----------------------------------------------------------------------- network -------------------------------------------------------------------------
	...
	local public = conky_parse("${execi 300 curl https://api.ipify.org}")
	-- arrow to network
	cairo_move_to(cr, item_startx, item_starty);
	cairo_curve_to(cr, item_curvex, item_curvey, item_curvex, item_curvey-70, item_endx, item_endy);
	set_color(1,0.5);
	...
	-- value text
	text = "Local IP: "..network;
	cairo_text_extents(cr, text, extents)
	cairo_move_to(cr, item_centerx - extents.width/2, item_centery + item_radius + item_font_size + 8);
	cairo_show_text(cr, text);

	-- public ip text
	text = "Public IP: "..public;
	cairo_text_extents(cr, text, extents)
	cairo_move_to(cr, item_centerx - extents.width/2, item_centery + item_radius + item_font_size*2+8);
	cairo_show_text(cr, text);
	...
	if(stats ~= "") then
		...
		_,nex,nmonth_received = string.find(stats, year.."%-"..month.."%s*(.-)iB",nex);
